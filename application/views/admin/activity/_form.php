<?php
$isNew = true;
if(segment(4) != '' || $this->session->userdata('activity_data')) {
    $data = segment(4) != '' ? $activity[0] : $this->session->userdata('activity_data');
    $isNew = false;
    $name = $data['activity'];
    $act_id = $this->common_model->get_where('tbl_act_dest', array('act_id' => $data['id']));
    $acts = array();
    if($act_id) {
        foreach($act_id as $a) {
            $acts[] = $a['dest_id'];
...
<div class="box-header">
    <h3 class="box-title">Activity Manager | <?php echo ($isNew) ? 'Add' : 'Update' ?></h3>
                <span class="pull-right add-new">
                    <a href="<?php echo base_url('admin/activity/add_update') ?>" class="btn btn-success">Add New</a>
                </span>
</div><!-- /.box-header -->
<div class="box-body">
                ...
                <table class="table table-bordered">
                    <tr>
                        <td>
                            ...
                            <input id="slug-source" type="text" class="form-control required" name="activity" value="<?php echo (!$isNew) ? $name : '' ?>">
                        </td>
                    </tr>
                    <tr>
                        <td>
                    ...
                    <tr>
                        <td>
                            <label>Country: </label>
				            <?php $country = $this->common_model->get_all('tbl_destinations');
				            foreach($country as $c) { ?>
                                <input <?php echo (!$isNew && in_array($c['id'], $acts)) ? 'checked' : '' ?> type="checkbox" value="<?php echo $c['id'] ?>" name="destination[]"> <?php echo $c['destination'] ?>
				            <?php } ?>
                        </td>
                    </tr>
                    <tr>
                        <td>
	                        ...
	                        <?php if(isset($banner_image) && $banner_image != '') { ?>
                                <img src="<?= base_url('images/activity/' . $banner_image) ?>" width="25%">
	                        <?php } ?>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if(isset($seo_image) && $seo_image != '') { ?>
                                <img src="<?= base_url('images/activity/' . $seo_image) ?>" width="25%">
                            <?php } ?>
                        </td>
                    </tr>
                    <tr>
